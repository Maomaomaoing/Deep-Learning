...
class VAE_model():
    ...
    def encode(self, img):
        img = tf.layers.conv2d(img, 32, 3, strides = (1,1), padding = 'same', kernel_initializer = self.init)
        img = tf.layers.batch_normalization(img, training = True) # (64, 64, 32)
        img = tf.nn.relu(img) 
        ...
        img = tf.layers.conv2d(img, 64, 3, strides = (1,1), padding = 'same', kernel_initializer = self.init)
        img = tf.layers.batch_normalization(img, training = True)# (32, 32, 64)
        img = tf.nn.relu(img)
		...
		img = tf.layers.conv2d(img, 128, 3, strides = (1,1), padding = 'same', kernel_initializer = self.init)
        img = tf.layers.batch_normalization(img, training = True)# (16, 16, 128)
        img = tf.nn.relu(img)
        ...
        img = tf.layers.batch_normalization(img, training = True)# (4, 4, 256)
        img = tf.nn.relu(img)
        
        img_shape = img.shape # (4, 4, 256)
        img = tf.layers.flatten(img) # (4, 4, 256) -> 4, 4, 256
        print(img.shape) # 4, 4, 256=8192
    ...
    def decode(self, img, img_shape):
        ...
        img = tf.layers.dense(img, 4096, kernel_initializer = self.init) # 4096 -> 16384
        img = tf.layers.batch_normalization(img, training = True)
        img = tf.nn.relu(img)
        
        img = tf.reshape(img, img_shape) # 8*8*128 -> (4, 4, 256)
        
		img = tf.layers.conv2d_transpose(img, 256, 3, strides = (2,2), padding = 'same', kernel_initializer = self.init)
        img = tf.layers.batch_normalization(img, training = True)# (8, 8, 256)
        img = tf.nn.relu(img)
		...
		img = tf.layers.conv2d_transpose(img, 64, 3, strides = (1,1), padding = 'same', kernel_initializer = self.init)
        img = tf.layers.batch_normalization(img, training = True)# (16, 16, 64)
        img = tf.nn.relu(img)
        ...
        img = tf.layers.conv2d_transpose(img, 32, 3, strides = (1,1), padding = 'same', kernel_initializer = self.init)
        img = tf.layers.batch_normalization(img, training = True)# (32, 32, 32)
        img = tf.nn.relu(img)
        ...
        img = tf.layers.conv2d_transpose(img, 3, 3, strides = (1,1), padding = 'same', kernel_initializer = self.init)
