...
#		 print(input_img.shape)
		...
		img = tf.layers.conv2d(input_img, 64, 5, padding = 'same', kernel_initializer = self.initial)
		img = tf.nn.relu(img)		 
		img = tf.layers.conv2d(img, 64, 5, padding = 'same', kernel_initializer = self.initial)
		img = tf.nn.relu(img)		
		img = tf.layers.max_pooling2d(img, 2, 2, 'same')
		
#		 print(img.shape)
		
		img = tf.layers.conv2d(img, 128, 5, padding = 'same', kernel_initializer = self.initial)
		img = tf.nn.relu(img)		 
		img = tf.layers.conv2d(img, 128, 5, padding = 'same', kernel_initializer = self.initial)
		img = tf.nn.relu(img)		
		img = tf.layers.max_pooling2d(img, 2, 2, 'same')
		
		img = tf.layers.conv2d(img, 256, 5, padding = 'same', kernel_initializer = self.initial)
		img = tf.nn.relu(img)		 
		img = tf.layers.conv2d(img, 256, 5, padding = 'same', kernel_initializer = self.initial)
		img = tf.nn.relu(img)		
		img = tf.layers.max_pooling2d(img, 2, 2, 'same')
		
		img = tf.layers.conv2d(img, 512, 5, padding = 'same', kernel_initializer = self.initial)
		img = tf.nn.relu(img)		 
		img = tf.layers.conv2d(img, 512, 5, padding = 'same', kernel_initializer = self.initial)
		img = tf.nn.relu(img)		
		img = tf.layers.max_pooling2d(img, 2, 2, 'same')
...
with tf.Session() as sess:
	...
	save_path = saver.save(sess, "model_k5.ckpt")
	
""" save file """
save_json(train_acc, "train_acc_k5.json")
save_json(train_loss, "train_loss_k5.json")

save_json(test_acc, "test_acc_k5.json")
save_json(test_loss, "test_loss_k5.json")
