...
class CNN_Model():
	...
	def __init__(self, optm, lr, batch_size, k):
		...
		def preprocess(filename):
			...
			img = tf.reshape(tf.image.resize_images(img, [200, 200]), [200, 200, 3]) 
...
#		 print(input_img.shape)
		...
		img = tf.nn.relu(img)		 
		img = tf.layers.conv2d(img, 3, 64, padding = 'same', kernel_initializer = self.initial)
		img = tf.nn.relu(img)		
		img = tf.layers.max_pooling2d(img, 2, 2, 'same')
		
#		 print(img.shape)
		
		img = tf.layers.conv2d(img, 3, 128, padding = 'same', kernel_initializer = self.initial)
		img = tf.nn.relu(img)		 
		img = tf.layers.conv2d(img, 3, 128, padding = 'same', kernel_initializer = self.initial)
		img = tf.nn.relu(img)		
		img = tf.layers.max_pooling2d(img, 2, 2, 'same')
		
		img = tf.layers.conv2d(img, 3, 256, padding = 'same', kernel_initializer = self.initial)
		img = tf.nn.relu(img)		 
		img = tf.layers.conv2d(img, 3, 256, padding = 'same', kernel_initializer = self.initial)
		img = tf.nn.relu(img)		
		img = tf.layers.max_pooling2d(img, 2, 2, 'same')
		
		img = tf.layers.conv2d(img, 3, 512, padding = 'same', kernel_initializer = self.initial)
		img = tf.nn.relu(img)		 
		img = tf.layers.conv2d(img, 3, 512, padding = 'same', kernel_initializer = self.initial)
		img = tf.nn.relu(img)		
		img = tf.layers.max_pooling2d(img, 2, 2, 'same')
...
filename2 = "D:/School/碩一/深度學習/DL_HW2/animal-10/train/butterfly/OIP-zqYnjyMopVxXFFJHoztbOQHaH7.jpeg"
...
x_path = tf.placeholder(tf.string, [None])
imgs = tf.reshape(tf.map_fn(preprocess, x_path, dtype = tf.float32), [-1, 200, 200, 3])
print(imgs.shape)
mean, var = tf.nn.moments(imgs, axes = [0, 1, 2])
std = tf.sqrt(var)
imgs = (imgs-mean)/std

img1 = tf.image.decode_jpeg(tf.read_file(filename2), channels=3)
img1 = tf.reshape(tf.image.resize_images(img1, [200, 200]), [200, 200, 3])
...
with tf.Session() as sess:
    img1 = sess.run(img1)
#    img, mean1, var1 = sess.run([imgs, mean, std], {x_path: data})
# 
